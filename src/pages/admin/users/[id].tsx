...
import { Module, UserWithPermissions } from '@/types';
...
export default function EditUserPage() {
  ...
  }, [session, status, router, isAdmin, id]);

  const fetchUser = async () => {
    try {
      const res = await fetch(`/api/users/${id}`);
      if (res.ok) {
        const data: UserWithPermissions = await res.json();
        setFormData({
          nome: data.nome,
          email: data.email,
          senha: '',
          grupo: data.grupo,
          cadastrar: data.permissions?.cadastrar || false,
          batepapo: data.permissions?.batepapo || false,
  ...
  const handleModuleToggle = (moduleId: number) => {
    setFormData((prev) => ({
      ...
      moduleIds: prev.moduleIds.includes(moduleId)
        ? prev.moduleIds.filter((mid) => mid !== moduleId)
  ...
  return (
    <>
      <Head>
        <title>Editar Usuário - Base de Conhecimento</title>
      </Head>
      <Layout>
